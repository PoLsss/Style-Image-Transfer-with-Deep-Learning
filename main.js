...
class Main {
  ...
  initializeStyleTransfer() {
    ...
    this.displayModal = document.getElementById('stylized');
    ...
    this.styleButton.onclick = () => {
      ...
      this.startStyling().finally(() => {
        ...
        console.log(this.checkGenerated)
        this.pushImageButton = document.getElementById('push-image');
        if (this.checkGenerated) {
          this.pushImageButton.disabled = false;
        } else this.pushImageButton.disabled = true;
      });
    };
    ...
    this.saveChanges = document.getElementById('save-changes');
    this.saveChanges.onclick = () => {
      // Lấy đối tượng canvas

      const displayImg = document.getElementById('display-img')
      console.log(displayImg)
      // Kiểm tra xem trình duyệt có hỗ trợ phương thức toDataURL() hay không
      if (!displayImg.toDataURL()) {
        console.log('Trình duyệt không hỗ trợ tải ảnh từ canvas.');
        return;
      }

      // Tạo một đối tượng hình ảnh mới
      const image = new Image();
      // Thiết lập nguồn ảnh là dữ liệu từ canvas
      image.src = displayImg.toDataURL();
      ...
      // Kích hoạt sự kiện click cho liên kết
      link.click();
    }
  ...
  async SeeImg() {
    ...
    // Khi người dùng nhấp ra bên ngoài modal, ẩn modal
    ...
    const displayImg = document.getElementById('display-img');
    await tf.browser.toPixels(this.displayModal, displayImg);
    // console.log(this.displayModal)
    this.displayModal.dispose();
  }
  ...
  initializeCombineStyles() {
    ...
    this.combdisplayModal = document.getElementById('c-stylized');
    ...
    this.combineButton.onclick = () => {
      ...
      this.startCombining().finally(() => {
        this.enableStylizeButtons();
        /////
        this.checkCombGenerated = true;
        console.log(this.checkCombGenerated)
        this.pushCombImageButton = document.getElementById('c-push-image');
        if (this.checkCombGenerated) {
          this.pushCombImageButton.disabled = false;
        } else this.pushCombImageButton.disabled = true;
      });
    };
    ...
    // Tiếp tục tab c-style
    this.pushCombImageButton = document.getElementById('c-push-image');
    this.pushCombImageButton.onclick = () => {
      this.combSeeImg();
    ...
    this.saveCombChanges = document.getElementById('c-save-changes');
    this.saveCombChanges.onclick = () => {
      // Lấy đối tượng canvas
      const combdisplayImg = document.getElementById('c-display-img')
      console.log(combdisplayImg)
      // Kiểm tra xem trình duyệt có hỗ trợ phương thức toDataURL() hay không
      if (!combdisplayImg.toDataURL()) {
        console.log('Trình duyệt không hỗ trợ tải ảnh từ canvas.');
        return;
      }
      // Tạo một đối tượng hình ảnh mới
      const image = new Image();
      image.src = combdisplayImg.toDataURL();
      console.log(image.src)
      const link = document.createElement('a');
      link.href = image.src;
      link.download = 'myCombImage.png';
      link.click();
    }
  }
  ...
  async combSeeImg() {
    await tf.nextFrame();
    const modalComDisplay = document.getElementById("c-myModal");
    modalComDisplay.style.display = "block";
    // Khi người dùng nhấp ra bên ngoài modal, ẩn modal
    window.onclick = function (event) {
      if (event.target == modalComDisplay) {
        modalComDisplay.style.display = "none";
      }
    }
    const combdisplayImg = document.getElementById('c-display-img');
    await tf.browser.toPixels(this.combdisplayModal, combdisplayImg);
    console.log('ssssss', this.combdisplayModal)
    this.combdisplayModal.dispose();
  }
  ...
  connectImageAndSizeSlider(img, slider, square) {
    slider.oninput = (evt) => {
      ...
      if (img.style.width) {
        // Ảnh resize về hình vuông
        // a fixed pixel value.
        img.style.width = img.height + "px";  // Tạo hình vuông
      }
    }
    if (square !== undefined) {
      square.onclick = (evt) => {
  ...
  setImage(element, selectedValue) {
    if (selectedValue === 'file') {
      ...
      console.log(randomNumber, links.length)
    } else {
  ...
  enableStylizeButtons() {
    ...
    this.styleButton.textContent = 'Tạo Ảnh';
    this.combineButton.textContent = 'Kết hợp kiểu';
  }
  ...
  async startStyling() {
    await tf.nextFrame();
    this.styleButton.textContent = 'Vui lòng chờ 1 ...';
    await tf.nextFrame();
    ...
    if (this.styleRatio !== 1.0) {
      this.styleButton.textContent = 'Vui lòng chờ 2...';
      await tf.nextFrame();
    ...
    this.styleButton.textContent = 'Đang tạo ảnh...';
    await tf.nextFrame();
    ...
    bottleneck.dispose();
    this.displayModal = stylized
    // console.log(this.stylized)
    // stylized.dispose();
  }
  ...
  async startCombining() {
    await tf.nextFrame();
    this.combineButton.textContent = 'Đang xử lý ảnh 1. Vui lòng chờ ...';
    await tf.nextFrame();
